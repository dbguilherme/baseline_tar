...
class EvalMeasure(object):
    ...
    def __init__(self, topic_id, num_docs, num_rels):
        ...
        self.outputs = {'topic_idxxx':0, 'num_docs':0, 'num_rels': 0}
...
class CountBasedMeasures(EvalMeasure):
    ...
    def finalize(self):
        ...
        print("xxxx  " , (float(self.num_docs)), " ",  self.last_rel_95, " ", float(self.num_docs))
...
class GainBasedMeasures(EvalMeasure):
    ...
    def __init__(self, topic_id, num_docs, num_rels):
        ...
        self.outputs = {'total_cg':1, 'max_cg':1, 'cgat': 2,
                        'threshold':1, 'norm_threshold':1, 'threshold_cg':1, 'threshold_ncg':1,'cost':0}
    ...
    def finalize(self):
        ...
        if self.threshold == self.num_docs:
            ...
            self.threshold = self.last_rank
            self.norm_threshold = float(self.threshold) / float(self.num_docs)
        #self.cost=float(self.shown) / float(self.num_docs)
        if self.max_cg > 0:
...
class CostBasedMeasure(EvalMeasure):
    ...
    def finalize(self):
        ...
        self.cost=self.num_shown /self.num_docs 
        Pw = 0
        for i in range(1, Mr):
